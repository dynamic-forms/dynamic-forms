...
describe('DynamicFormFieldModule', () => {
  const libraryProvider = { provide: DynamicFormLibraryService, useValue: new DynamicFormLibraryService(dynamicFormLibrary) };
  const testModules: { name: string; def: TestModuleMetadata }[] = [
    {
      name: 'DynamicFormFieldModule',
      def: { imports: [DynamicFormFieldModule], providers: [libraryProvider] },
    },
    {
      name: 'withDynamicFormFieldDefaultFeatures',
      def: { providers: [...importDynamicFormsProviders(...withDynamicFormFieldDefaultFeatures()), libraryProvider] },
    },
  ];
