...
export class DynamicFormMarkdownService {
  ...
  parse(markdown: string, options?: DynamicFormMarkdownOptions): string {
  ...
  load(source: string, options?: DynamicFormMarkdownOptions): Observable<string> {
    return this.httpClient.get(source, { responseType: 'text' }).pipe(
      map(markdown => this.parse(markdown, options))
    );
  }
